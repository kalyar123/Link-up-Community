...
const Memberblooddonations = () => {
  ...
  useEffect(() => {
    axios
      .get('http://localhost:9000/member/blood-donate/get')
      .then(res => {
        console.log(res);
        setUsers(res.data);

        // Extract unique blood groups from the response data
        const uniqueBloodGroups = [...new Set(res.data.map(user => user.blood))];
            ...
            <div class="row page-titles">
              ...
              <ul className="nav nav-pills">
                {bloodGroups.map(bloodGroup => (
                  <li className="nav-item" key={bloodGroup}>
                    <button
                    style={{backgroundColor:'darkcyan',marginLeft:'3px'}}
                      className={`nav-link ${activeBloodGroup === bloodGroup ? 'active' : ''} `}
                      onClick={() => handleBloodGroupSelection(bloodGroup)}
                    >
                      {bloodGroup}
                    </button>
                  </li>
              ...
              <div className="col-12">
                <div className="card">
                  <div className="card-body">
                    <h4 className="card-title">My blood donation history</h4>
                    ...
                    <tbody>
                      {users
                        .filter(user => activeBloodGroup === '' || user.blood === activeBloodGroup)
                        .map(user => (
                          <tr key={user.id}>
                            <td>{user.name}</td>
                            <td>{user.mosque}</td>
...
export default Memberblooddonations
