...
const User_blood_request = () => {
  ...
  const {name,mosque,age,phoneno,unit,blood,email}=user;
  ...
  const onSubmit = async (e) => {
    ...
    if (formIsValid) {
      try {
        await axios.post('http://localhost:9000/blood-request', user);
        alert('Data inserted');
      } catch (error) {
        console.log(error);
      }
...
export default User_blood_request
