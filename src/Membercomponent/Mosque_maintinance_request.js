...
const Mosque_maintinance_request = () => {
 ...
 const onSubmit = async (e) => {
    ...
    // } else if (!/^[a-zA-Z0-9\s]+$/.test(reason)) {
    //   formIsValid = false;
    //   newErrors.reason = "Please enter a valid reason (only alphabetic characters and numbers)";
    // }
    ...
    if (formIsValid) {
      try {
        await axios.post('http://localhost:9000/member/maintinance', user);
        alert('Data inserted');
      } catch (error) {
        console.log(error);
      }
...
export default Mosque_maintinance_request
